...
int valorKNN(int k_valor, vector<pair<double, int>> distancias) {
    ...
    for (int i = 0; i < k_valor; i++) {
        ...
        // cout << "Pos: " << i << " " << distancias[i].first << " " << distancias[i].second <<
        // endl;
        res[l]++;
    }
    ...
    label = pos_max;
    // cout << "N_0 " << res[0] << "\tN_1 " << res[1] << "\tN_2 " << res[2] << "\tElegido " << label
    // << endl;
...
Particula::Particula() {
    ...
    cont = 0;
    // cout << "Sizes: " << data_test.size() << " " << data_training.size() << " " <<
    // labels_test.size() << " " << labels_training.size() << endl;
}
...
void Particula::actualizarVelocidad(vector<int> &g) {
    ...
    for (int i = 0; i < dimension; i++) {
        vel.at(i) = inercia * vel.at(i) + c_cog * rd1.at(i) * (b_pos.at(i) - pos.at(i)) +
                    c_social * rd2.at(i) * (g.at(i) - pos.at(i));  // Con componente aleatoria
        // vel.at(i) = inercia*vel.at(i) + c_cog*(b_pos.at(i) - pos.at(i)) + c_social*(g.at(i) -
        // pos.at(i)); //Sin componente aleatoria. No satisfactorio
...
void Particula::actualizarVelocidad(vector<int> &g){
    ...
    for (int i = 0; i < dimension; i++){
        vel.at(i) = inercia*vel.at(i) + c_cog*rd1.at(i)*(b_pos.at(i) - pos.at(i)) +
c_social*rd2.at(i)*(g.at(i) - pos.at(i));
    }
...
#pragma omp simd
    ...
    // cout << "Valor: " << valor << "\t N_aciertos " << n_aciertos << "\tN_fallos " << n_fallos <<
    // endl;
    return valor;
}
...
int Particula::getBK() { return mejor_k; }
