...
for N in $( eval echo {6..$NMAXNODOS} );
do
    ...
    do
        ...
        echo -e "\t" $R
        salloc -p guest -N$N -n$N -w compute-0-4,compute-0-1,compute-0-2,compute-0-3,compute-0-6,compute-0-7 mpiexec --map-by node --bind-to none -x OMP_NUM_THREADS=12 ./pso > last.txt
